...
public class TupaImpl extends Conexion implements ICrud<Tupa>, IReporte<Tupa> {
    ...
    public void editar(Tupa tupa) throws Exception {
        try {
            ...
            PreparedStatement ps = this.conectar().prepareStatement(sql);
            ps.setString(1, tupa.getNUMTUP());
            ps.setString(2, tupa.getNOMTUP().toUpperCase());
            ps.setDouble(3, tupa.getPRETUP());
            ps.setInt(4, tupa.getPLATUP());
            ps.setInt(5, tupa.getArea().getIDARE());
    ...
    public void generarReporteIndividual(Map parameters) throws Exception {
        File jasper = new File(FacesContext.getCurrentInstance().getExternalContext().getRealPath("Reportes/Tupa/TUPA_R.jasper"));
