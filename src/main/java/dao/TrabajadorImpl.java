...
public class TrabajadorImpl extends Conexion implements ICrud<Trabajador>, IReporte<Trabajador> {
    ...
    public void cambiarArea(Trabajador t) throws Exception{
        try {
            String sql = "UPDATE TRABAJADOR SET IDARE=? WHERE IDTRAB=?";
            PreparedStatement ps = this.conectar().prepareStatement(sql);
            ps.setInt(1, t.getArea().getIDARE());
            ps.setInt(2, t.getIDTRAB());
            ps.executeUpdate();
            ps.clearParameters();
            ps.close();
        } catch (Exception e) {
    ...
    public void resetearContra(Trabajador modelo) throws Exception {
        try {
            System.out.println("RESET->"+modelo.getPersona().getDNIPER());
