...
public class ActaImpl extends Conexion implements ICrud<Acta>, IReporte<Acta> {
    ...
    public void registrar(Acta modelo) throws Exception {
        try {
            ...
            ps.setDate(6, new java.sql.Date(modelo.getFECREGACTA().getTime()));
            ps.setString(7, modelo.getOBSACTA());
            ps.setString(8, modelo.getUbigeo().getCODUBI());
            ps.setString(9, modelo.getDIRACT());
            ps.setDate(10, new java.sql.Date(modelo.getFECACT().getTime()));
    ...
    public void editar(Acta modelo) throws Exception {
        try {
            String sql = "UPDATE ACTA SET NUMLIBACTA=?, NUMFOLACTA=?, "
                    + "FECREGACTA=?, OBSACTA=?, "
                    + "CODUBI=?, DIRACT=?, FECACT=? "
                    + "WHERE IDACTA=?";
            PreparedStatement ps = this.conectar().prepareStatement(sql);
            ps.setString(1, modelo.getNUMLIBACTA());
            ps.setString(2, modelo.getNUMFOLACTA());
            ps.setObject(3, modelo.getFECREGACTA(), java.sql.Types.DATE);
            ps.setString(4, modelo.getOBSACTA());
            ps.setString(5, modelo.getUbigeo().getCODUBI());
            ps.setString(6, modelo.getDIRACT());
            ps.setObject(7, modelo.getFECACT(), java.sql.Types.DATE);
            ps.setInt(8, modelo.getIDACTA());
            ps.executeUpdate();
            ps.clearParameters();
            ps.close();
        } catch (Exception e) {
