...
public class ContactoC implements Serializable {
    ...
    public void enviarMensaje() {
        try {
            contacto.setPhone(servicios.SesionS.getSesion().getTrabajador().getPersona().getCELPER());
    ...
    public void enviarMensajeDesarrolladores() {
        try {
            contacto.setFrom(contacto.getUser());
            FacesContext.getCurrentInstance().addMessage(null,
                    new FacesMessage(FacesMessage.SEVERITY_INFO, daoContacto.enviarMensajeDesarrolladores(contacto), null));
            contacto.clear();
        } catch (Exception e) {
            e.printStackTrace();
        }
