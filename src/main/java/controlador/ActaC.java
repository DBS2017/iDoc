...
public class ActaC implements Serializable {
    ...
    public void listarActa(Acta a) throws Exception {
        try {
            listaDetalle = daoDetalle.listar(a);
            cabeceraSeleccionado = listaDetalle.get(0).getActa();
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    /**
     * Comprueba si un acta ya existe
     *
    ...
    public void editarCabecera() throws Exception {
        try {
            daoActa.editar(cabeceraSeleccionado);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
        ...
        try {
            if (comprobar() == true) {
                if (listaCabecera.contains(cabecera) == false) {
                    ...
                    PrimeFaces.current().executeScript("enviar('Acta');");
//                    listar();
...
//                    listaDetalleFiltrado.clear();
    ...
    public void eliminar(Acta a) throws Exception {
        try {
            daoActa.eliminar(a);
            listar();
            FacesContext.getCurrentInstance().addMessage(
